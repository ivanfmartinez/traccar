 ...
 *
 * ID can be get from URI path - /csv/ID Authorization header - USER:PASSWORD
 * mapped to (ID)
 *
 */
public class CarStatsViewerDecoder extends BaseHttpProtocolDecoder {
    ...
    /**
     * Use specific path for requests, this way can be processed by same SSL server
     * as traccar web interface
     */
    ...
    private class CSVProcessor {
        ...
        // traccar have "batteryLevel" for percentage, because of this the name must be
        // different
        ...
        CSVProcessor(final DeviceSession deviceSession) {
        ...
        private abstract class CSVContext {
            ...
            protected Position processLocation(final JsonObject jsonRoot) {
                ...
                if (jsonRoot.containsKey(CSV_STATE_OF_CHARGE)) {
                    ...
                    position.set(Position.KEY_BATTERY_LEVEL,
                            getDoubleFromJson(jsonRoot, CSV_DRIVING_POINT_STATE_OF_CHARGE) * 100);
                }
            ...
            protected void setFromJson(final Position position, final JsonObject object, final String setKey,
                    final String getKey) {
            ...
            protected void setDoubleFromJson(final Position position, final JsonObject object, final String setKey,
                    final String getKey) {
                if (object.containsKey(getKey)) {
            ...
            protected void processExtraData(final Position position, final JsonObject jsonRoot) {
                ...
                if (StringUtils.isNotBlank(ignitionState)) {
                    position.set(Position.KEY_IGNITION,
                            "started".equalsIgnoreCase(ignitionState) || "on".equalsIgnoreCase(ignitionState));
                ...
                final String powerMilliWatts = getFromJson(jsonRoot, CSV_POWER);
                ...
                if (StringUtils.isNotEmpty(powerMilliWatts) && StringUtils.isNumeric(powerMilliWatts)) {
                    // it is charging when power value is negative
                    final boolean isCharging = "true".equals(chargePortConnected)
                            && StringUtils.isNotEmpty(powerMilliWatts)
                            && powerMilliWatts.startsWith("-");
                    position.set(Position.KEY_CHARGE, isCharging);
                    position.set(TRACCAR_POWER_M_W, Long.parseLong(powerMilliWatts));
                }
        ...
        public class CSVContextDrivingPoint extends CSVContext {
            ...
            CSVContextDrivingPoint(final Position rootPosition, final boolean singlePoint) {
            ...
            protected void processExtraData(Position position, JsonObject jsonRoot) {
                ...
                // If have same information with single drivepoint set data on main position
                // object
                // Drive points does not came with speed, in this case will create two
                // positions, one without speed
                // Ignoring and merging data will make history cleaner
                if (singlePoint && (position.getAltitude() == rootPosition.getAltitude())
                        && (position.getLatitude() == rootPosition.getLatitude())
                        && (position.getLongitude() == rootPosition.getLongitude()) && (Math.abs(
                                position.getDeviceTime().getTime() - rootPosition.getDeviceTime().getTime()) < 1000)) {
                    ignore = true;
    ...
    protected Object decode(Channel channel, SocketAddress remoteAddress, Object msg) throws Exception {
        ...
        final DeviceSession deviceSession = getDeviceSession(channel,
                StringUtils.isNotBlank(realIP) ? new InetSocketAddress(realIP, 1) : remoteAddress, authID, uriID);
